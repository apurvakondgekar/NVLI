...
//				     }
				...
				WebElement categorybar = driver.findElement(By.cssSelector("#block-views-block-item-type-block-1"));
				WebElement categorydraggable = categorybar.findElement(By.className("draggable"));


				//code for finding the category and then clicking on it

				for (String cat : uniqueElements)
				{
					List <WebElement> activeslide = categorybar.findElements(By.className("lead"));



				for (WebElement target : activeslide)
				{
					WebElement icon = target.findElement(By.linkText(cat));
